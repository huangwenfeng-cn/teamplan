...
def clear_data(onboard=None):
	doctypes = frappe.get_all("DocType", filters={"module": "teamplan"}, pluck="name")
	for doctype in doctypes:
		frappe.db.delete(doctype)

	admin = frappe.get_doc("User", "Administrator")
	admin.add_roles("teamplan Admin")
	...
	if not frappe.db.exists("User", "system@example.com"):
		frappe.get_doc(
			...
			last_name="User",
			send_welcome_email=0,
			roles=[{"role": "teamplan Admin"}, {"role": "System Manager"}],
		).insert()
