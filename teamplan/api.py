...
import teamplan
from teamplan.utils import validate_type
...
def get_user_info(user=None):
	...
	filters = {"roles.role": ["like", "teamplan %"]}
	if user:
		filters["name"] = user
	...
	for user in users:
		...
		user.role = None
		for role in ["teamplan Guest", "teamplan Member", "teamplan Admin"]:
...
def change_user_role(user: str, role: str):
	if teamplan.is_guest():
		frappe.throw("Only Admin can change user roles")

	if role not in ["teamplan Guest", "teamplan Member", "teamplan Admin"]:
		return get_user_info(user)[0]

	user_doc = frappe.get_doc("User", user)
	for _role in user_doc.roles:
		if _role.role in ["teamplan Guest", "teamplan Member", "teamplan Admin"]:
...
def invite_by_email(emails: str, role: str, projects: list = None):
	...
	existing_invites = frappe.db.get_all(
		...
		filters={
			"email": ["in", email_list],
			"role": ["in", ["teamplan Admin", "teamplan Member"]],
		},
		pluck="email",
	)

	if role == "teamplan Guest":
...
def get_unsplash_photos(keyword=None):
	from teamplan.unsplash import get_by_keyword, get_list
...
def onboarding(data):
	...
	invite_by_email(emails, role="teamplan Member")
	return team.name
...
def search(query, start=0):
	from teamplan.search import teamplanSearch

	search = teamplanSearch()
