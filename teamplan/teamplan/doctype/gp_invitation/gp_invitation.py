...
class GPInvitation(Document):
	def before_insert(self):
		...
		if self.role == "teamplan Guest" and not (self.teams or self.projects):
			frappe.throw("Project is required to invite as Guest")

		if self.role != "teamplan Guest":
			self.teams = None
			self.projects = None
	...
	def invite_via_email(self):
		invite_link = frappe.utils.get_url(f"/api/method/teamplan.api.accept_invitation?key={self.key}")
		if frappe.local.dev_server:
			print(f"Invite link for {self.email}: {invite_link}")

		title = "teamplan"
		template = "teamplan_invitation"
